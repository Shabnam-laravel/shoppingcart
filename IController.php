...
class IController extends Controller
{
    public function index1(){
        ...
        $product = Product::all();
        return view('enest/index1',compact('category','product'));
    }
    ...
    public function display_category($id){
        ...
        return view('enest/products1',compact('data','category','cat_name'));
    }
    public function buy_product($id){
        $category = Category::all();
        $product = Product::where('id',$id)->get();
        return view('enest/buy-product1',compact('product','category'));
    }
    public function new_products(){
        $category = Category::all();
        $data=Product::orderby('id','desc')->get();  
        return view('enest/new-product1',compact('data','category'));
    }
    public function special_products(){
        $category = Category::all();
        $data=Product::where('special','=',1)->get();
        return view('enest/special1',compact('data','category'));
    }
    public function all_products(){
        $category = Category::all();
        $data=Product::inRandomOrder()->get();
        return view('enest/all-products1',compact('data','category'));
    }
    public function contact(){
        $category = Category::all();
        return view('enest/contact1',compact('category'));
    }
    public function login1(){
        return view('enest/login1');
    }
    ...
    public function checkout($uid)
	{
		...
		->select('cart.id','product.productname','signup.fullname','cart.qty','product.productprice')
		->where('cart.userid','=',$uid)->get();
        $category = Category::all();
        return view('enest/checkout1',compact('check','category'));
	}
    ...
    public function search(Request $req){
        ...
        $category = Category::all();
        return view('enest/search1',compact('category','data'));
    }
}
